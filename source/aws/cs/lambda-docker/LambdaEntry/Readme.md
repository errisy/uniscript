...
WORKDIR /src
COPY ["LambdaEntry.csproj", "LambdaEntry/"]
RUN dotnet restore "LambdaEntry/LambdaEntry.csproj"

WORKDIR "/src/LambdaEntry"
COPY . .
RUN dotnet build "LambdaEntry.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "LambdaEntry.csproj" \
...
Execute unit tests
```
cd "LambdaEntry/test/LambdaEntry.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "LambdaEntry/src/LambdaEntry"
