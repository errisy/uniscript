import { Namespace, Service, Message, Method, Property, Parameter, Type, VoidType, ServiceInterface, MessageInterface, StringType, Enum } from './definitions';
...
`    }
    ...
    class EnumEmitter {
        ...
        emitEnum(builder: CodeBuilder, indent: number) {
            builder.addHierarchicalImport('enum', 'Enum');
            builder.appendLine('', indent);
            builder.appendLine('', indent);
            builder.appendLine(`class ${this.instance.Name}(Enum):`, indent);
            for (let name of this.instance.Fields.keys()) {
                let value = this.instance.Fields.get(name);
                if (typeof value == 'string') {
                    builder.appendLine(`${name} = '${value}'`, indent + 1);
